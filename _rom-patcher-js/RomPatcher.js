...
var romFile, headeredRomFile, unheaderedRomFile, patch, romFile1, romFile2, tempFile, zc;
/* Shortcuts */
...
function openPatchFile(f){zc=f.value.replace(/.*[\/\\]/, '').replace(/\.[^/.]+$/, '');tempFile=new MarcBinFile(f, _readPatchFile);}
function applyPatchFile(p,r){
	if(p && r){
		var patchedROM=p.apply(r);
		patchedROM.fileName=zc + ".wad";
